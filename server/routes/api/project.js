...
router.get('/:id', passport.authenticate('jwt',{ session: false }) ,async (req, res,next) => {
    try {
        //finding a user 
        let projects = await Project.find({owner: req.params.id}, function (err, project) {
...
//@access   private
router.get('/project/:id', passport.authenticate('jwt',{ session: false }) ,async (req, res,next) => {
    console.log('fetch Project ID:', req.params.id)
    try {
        //finding a user 
        let projects = await Project.findOne({_id: req.params.id}, function (err, project) {
            if(err) {
