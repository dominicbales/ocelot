...
import {Input, Message, Button, Icon, Table, Pagination} from 'semantic-ui-react'
    ...
    render() {
        ...
        if(docs) {
            issuesList = docs.map((val) => {
                const issueIcon = val.active === 'active' ? 'check circle outline' : 'ban';
                const issueColor = val.active === 'active' ? 'green' : 'red';
    
                return (
                    <Table.Row style={{height: '75px', cursor: 'pointer'}} key={val._id}>
                        <Table.Cell collapsing>
                            <NavLink to={{
                                ...
                                <div>
                                    ...
                                    {val.issueName}
                                    <h6 style={{margin: '0'}}>Created by {val.ownerName} {moment(val.createdAt).startOf().fromNow()}</h6>
                                </div>
                            </NavLink>
                        </Table.Cell>
                    </Table.Row>
        ...
        return (
            <div style={{overflowY: 'auto'}} className='flex flex-column flex-align-items-center'>
                <Message style={{width: '50%', marginTop: '20px', minHeight: '110px'}}>
                    <Message.Header>Changes in contributing</Message.Header>
                    <p>
                    We updated our contribute policy here to better service our customers. We recommend reviewing the
                    changes.
                    </p>
                </Message>
                <div className='flex flex-justify-between' style={{width: '50%'}}>
                    <Input transparent  icon='search' placeholder='Search...' />
                    <Button onClick={this.handleOpenModal} style={{marginLeft: '20px'}}>New Issue</Button>
                </div>
                ...
                <Pagination onPageChange={this.handlePageChange} defaultActivePage={1} totalPages={issues.totalPages} />
                {extend &&
                <AddIssueModal 
                project={project} 
                user={user}
